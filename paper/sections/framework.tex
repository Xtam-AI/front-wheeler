...
\fw{}($\tau$, $\kappa_{\max}$) (trigger threshold and cooldown cap),
\bcd{} (as \fw{} with trunk-only bursts), \textsc{ftlp}($s$) (the reverse
of \lpft{}: full steps for the first fraction $s$ of training, front steps
after), and \textsc{pburst}($P$) (open-loop bursts: $B$ consecutive full
steps every $P$ steps, no trigger). Cost is reported on two axes:
...
$c_{\mathrm{full}}$. Concretely: the loss EMA is $\bar L_t = (1{-}\alpha)\bar L_{t-1} +
\alpha L_t$ with $\alpha = 0.02$; at each check (every $C$ front steps,
once past the cooldown), the trigger computes the relative improvement
$r = (\bar L_{\mathrm{prev}} - \bar L_t)/|\bar L_{\mathrm{prev}}|$
against the EMA at the previous check, and fires a burst of $B$ consecutive
full steps when $r < \tau$; the same statistic evaluated over the burst
itself decides the backoff. \fw{} (Algorithm~\ref{alg:fw}) therefore
treats each burst as both a progress step and a \emph{measurement}: an unproductive
...
We evaluate in two stages: controlled MLPs under SGD
(Section~\ref{sec:exp2}), then a transformer under AdamW, where the
optimizer itself enters the mechanism (Section~\ref{sec:exp3}); the wiring
study that supplied the method's premise is reported in
Appendix~\ref{sec:exp1}.

\begin{algorithm}[t]
\caption{\fw{}: plateau-triggered bursts with cried-wolf backoff}
\label{alg:fw}
\begin{algorithmic}[1]
\State EMA loss $\bar L$; check interval $C$; threshold $\tau$; burst
length $B$; cooldown $\kappa \gets C$
\For{each minibatch}
  \If{in a burst}
    \State take a \textbf{full} step
    \If{the burst just ended}
      \If{the burst's relative EMA improvement ${<}\tau$}
        \State $\kappa \gets \min(2\kappa, \kappa_{\max})$
        \Comment{back off}
      \Else
        \State $\kappa \gets C$
      \EndIf
    \EndIf
  \Else
    \State take a \textbf{front} step (backward through the front only)
    \State \emph{(optional, shift-aware:)} \textbf{if} $\bar L$ jumps
      above its running minimum \textbf{then} $\kappa \gets C$
      \Comment{governor reset; \S\ref{sec:controls}}
    \If{$\ge \kappa$ front steps since the last burst, checked every $C$}
      \If{relative EMA improvement since the last check ${<}\tau$}
        \State start a burst of $B$ full steps
      \EndIf
    \EndIf
  \EndIf
\EndFor
