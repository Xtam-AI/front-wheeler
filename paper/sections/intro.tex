...
\begin{enumerate}
  ...
  weights matches dense training on MNIST ($98.2\%$ vs.\ $98.7\%$) and
  \emph{beats} it on CIFAR-10 ($55.9\%$ vs.\ $53.0\%$), where dense MLP
  training overfits (Table~\ref{tab:exp1}).
  ...
  CIFAR-10. A global \emph{scalar} does not suffice: \rmhebb{} learns
  ($2.6$--$5\times$ chance) but plateaus far below all vector-signal rules.
  The threshold for effective wiring search sits between a broadcast vector
  and a broadcast scalar.
  \item \textbf{Adaptive beats scheduled deep credit assignment.} At a matched
  budget of ${\sim}4$--$5\%$ full-BP steps, \fw{} beats the best periodic
  ...
  (b)~\emph{Isolated} full-BP steps interleaved with front training collapse
  the network: the front-trained, confident head back-propagates error
  signals into the stale trunk that we measure at ${\sim}20\times$ the
  head-gradient norm, driving dead-ReLU collapse unless gradients are
  clipped. The latter is an independent, from-scratch manifestation of the
