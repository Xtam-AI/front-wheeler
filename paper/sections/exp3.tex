...
\paragraph{Interleaving toxicity worsens under Adam.} Periodic
interleaving is destructive at every budget
...
\textsc{ftlp}($0.06$) is the worst cell among non-interleaved schedules
in our study ($2.290 \pm 0.035$, far below \textsc{front}): $300$ full steps from initialization---before
...
C source code; the post-shift front-to-full gap is $0.57$ of loss, vs.\
$0.35$ stationary). (An earlier mild-shift run used a $7\times$ smaller
replacement corpus and was superseded because \textsc{full} overfit it,
breaking the anchors; its logs are retained in the repository under the
\texttt{shift} tag.)
...
again edges the fair-placed clock ($1.452 \pm 0.002$), while \textsc{ftlp} lands
...
\paragraph{A shift-aware trigger.} Under the mild shift, plain \fw{}
re-escalates adequately on its own (recovering $17\%$ of the post-shift
front-to-full gap, essentially its stationary $18\%$). The harsh shift
exposes the weakness: the backoff, stretched long by pre-shift saturation,
pins plain \fw{}'s budget at $4.0\%$ even though the world has changed
and far more depth is warranted. The repair is one line: reset the
cooldown when the loss EMA \emph{jumps above} its running minimum---a
shift signature, the opposite of the plateau the trigger watches for.
Under the harsh shift this \emph{governor reset} self-selects a $6.7\%$
deep-step budget and reaches $1.411 \pm 0.005$---the best cheap schedule
