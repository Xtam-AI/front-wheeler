...
burst itself proves unproductive.

On MLPs (MNIST, CIFAR-10), \fw{} at a
...
\lpft{} spending $50\%$, and at low budget only burst-structured schedules meaningfully improve on
never back-propagating deep at all; a single backoff parameter further
...
every tested budget on the transformer. Under two mid-training distribution shifts of different severity, \fw{}
leads or ties the cheap field, invariant to when the shift occurs; a one-line
\emph{shift-aware governor reset} repairs its one documented weakness
(delayed re-escalation), making it the best cheap schedule under the
harshest condition we test. Across all conditions the pattern is consistent:
...
much as shifted ones; \fw{}, with one configuration throughout, is first or second among
cheap schedules at every low-budget and shifted condition, never falls
below the never-deep floor at any tested budget, and cedes only the
stationary mid-budget band---to schedules whose required timing is
catastrophic elsewhere. Three mechanism findings accompany the
...
is what makes deep steps safe; (ii) the damage is \emph{optimizer-dependent}: gradient clipping cures
it under SGD but not under Adam, whose scale-invariant updates neutralize
clipping; \fw{}'s burst structure protects it organically, which unlocks a
purely \emph{episodic} optimizer---trunk moment estimates discardable
between bursts at no quality cost, a systems win rather than a convergence
requirement; (iii) ablations localize the benefit to burst \emph{timing} rather than
block structure; the closed loop's contribution is discovering the schedule
without hindsight, and robustness when assumptions break.
